...
interface PasswordItemProps {
  password: Password;
  onEditItem(password: Password): void;
  onDeleteItem(password: Password): void;
}

export const PasswordItem = memo(
  ({ password, onDeleteItem, onEditItem }: PasswordItemProps) => {
    const { passwords, mutate } = usePasswordsContext();
      ...
      <>
        <tr className="bg-white border-b last:border-none transition duration-300 ease-in-out hover:bg-gray-100 relative">
          ...
          <td className="justify-end md:flex text-sm text-gray-900 font-light px-2 md:px-6  py-4 md:min-w-[200px] min-w-0">
            <Button
              onEditItem={() => onEditItem(password)}
              onDeleteItem={() => onDeleteItem(password)}
            />
          </td>
        </tr>
      </>
